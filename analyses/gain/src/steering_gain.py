...
if create_new:
    ...
    if assay == '38mm_projector':
        df0_all = gf.assign_paint_conditions(df0_all, meta1)
        
    # Save transformed data
    df0_all.to_pickle(output_fpath)
    print("Transformed data saved to {}".format(output_fpath))  

else:
    # Load existing data (prefer parquet; convert pkl to parquet if needed)
    pkl_path = local_fpath if (load_local and local_fpath is not None and os.path.exists(local_fpath)) else output_fpath
    parquet_path = pkl_path.replace('.pkl', '.parquet') if pkl_path.endswith('.pkl') else pkl_path + '.parquet'
    df0_all = gf.load_transformed_data(parquet_path)
    print("Loaded transformed.")

df0_all[['species', 'acquisition']].drop_duplicates().groupby('species')['acquisition'].count()
...
#%% 
# Only include assays with NO painting
df0 = df0_all[(df0_all['paint_coverage']=='none')].copy()
...
#%% 
# TMP: ADD TARGET_VEL
if 'target_vel' not in df0.columns or df0['target_vel'].isna().all():
    ...
    for fn, df_ in df0.groupby('file_name'):
        f1_ = df_[df_['id']==0]
        f2_ = df_[df_['id']==1]
        df0.loc[f1_.index, 'target_vel']     = f2_['vel'].values
        df0.loc[f1_.index, 'target_ang_vel'] = f2_['ang_vel'].values
        df0.loc[f2_.index, 'target_vel']     = f1_['vel'].values
        df0.loc[f2_.index, 'target_ang_vel'] = f1_['ang_vel'].values

#%%
#print(fn)
# '20250925-1632_fly5_Dyak-p1_3do_gh_2dR' # Example
# '20241213-1415_fly3_Dmel-p1_2do_gh_2dR' # No flyid=2?
# '20241218-1015_fly2_Dmel-p1_2do_gh_2dR' # No baseline?
# '20241218-1050_fly4_Dmel-p1_2do_gh_2dR' # No baseline, stimulus looks 
# '20250106-1501_fly1_Dmel-wt_3do_gh_3dR' # No baseline
# 20250106-1522_fly2_Dmel-LC10aS_3do_gh_3dR - 
# 20250108-1034_fly1_Dmel-LC10aS_2do_gh_2dR
# 20250117-0943_fly1_Dmel-LC10aS_gh_2do_2dR
fn = '20250205-1607_fly5_Dmel-p1-left-paint_2do_gh_2dR_2ccw' # fly IDs?



fn = df0['file_name'].unique()[100]
print(fn)
df_ = df0[df0['file_name']==fn].copy()
fig, ax = plt.subplots()
sns.scatterplot(data=df_[df_['id']==0], 
            x='stim_hz', y='target_vel', ax=ax,
            hue='stim_hz', s=5) # legend=1)
ax.set_ylim([-10, 200])

#%
fig, axn = plt.subplots(1, 2, sharex=True, sharey=True)
sns.scatterplot(data=df_[(df_['id']==1) & (df_['stim_hz']==0)], 
            x='pos_x', y='pos_y', ax=axn[0],
            hue='vel', s=5) # legend=1)
sns.scatterplot(data=df_[(df_['id']==1) & (df_['stim_hz']==0)], 
            x='pos_x', y='pos_y', ax=axn[1],
            hue='sec', s=5) # legend=1) 

#%
fig, ax = plt.subplots()
sns.scatterplot(data=df_[(df_['id']==1) & (df_['stim_hz']==0)],
            x='sec', y='pos_y', ax=ax, edgecolor='none', s=5)
...
if calculate_target_direction:
    ...
    for a, f1_ in f1.groupby('acquisition'):
        ...
        f1_['target_angular_position'] = -1*util.wrap_pi(np.arctan2(f1_['targ_centered_x'], f1_['targ_centered_y']))

        f1_['target_angle_diff'] = gf.get_heading_diff(f1_, 'target_angular_position',
...
chase_ = gf.bin_by_object_position(chase_, start_bin=-180, end_bin=180, bin_size=20)
chase_.reset_index(drop=True, inplace=True)

#chase_['binned_theta_error_num'] = chase_['binned_theta_error'].astype(int)
chase_['binned_theta_error_num'] = pd.to_numeric(chase_['binned_theta_error'], errors='coerce')
...
# Save
figname = f'gain_{yvar}_filter-chase-{filter_chase}_{hue_var}-{hue_str}_{species_str}'
...
#if assay == '38mm_projector':
grouper.extend(['stim_direction', 'pr_direction']) 
...
min_vel = round(np.nanmin(chase_['target_vel']))
max_vel = round(np.nanmax(chase_['target_vel']))
